...
function inicializadorEventos()
{
    ...
    drapandrop();//Inicio los eventos drag and drop
...
/*Funcion que inicia el proceso drap and drop*/
function drapandrop()
{
    var imagenes = document.getElementsByTagName("img");//Capturo todas las imagenes
   

    for (var i = 0; i < imagenes.length; i++) {//Para las imagenes

        console.log(imagenes[i].dataset.identificador);
        imagenes[i].draggable = true;//Las pongo que sean arrastables

        imagenes[i].addEventListener("dragstart", imagenesStartDrag);
        imagenes[i].addEventListener("dragend", imagenesEndDrag);

    }

//Para el div de destino
 var divCarrito = document.getElementById("carrito")//Capturo el div donde quiero dejar las imagenes
    divCarrito.addEventListener("dragover", dragOverCarrito);
    divCarrito.addEventListener("drop", dropCarrito);
...
function imagenesStartDrag(evento)
{
    
    var e = evento || window.event;
    e.dataTransfer.setData("text/html", this.dataset.identificador);  
}

function imagenesEndDrag(evento) {
    var e = evento || window.event;
}

//Funciones para el carrito(destino de las imagenes)
function dragOverCarrito(evento)
{
    var e = evento || window.event;
    e.preventDefault();
}
...
function dropCarrito(evento)
{
    var e = evento || window.event;
    var identificador=e.dataTransfer.getData("text/html");//Obtengo el identificador del producto que el pase en el dragStart
...
function crearTienda()
{
    ...
    for (var i = 0; i < tienda.numeroProductos(); i++) {//Me haces la estructura aqui metida tantas veces como elementos tengo el array productos de la tienda
        ...
        imagen.dataset.identificador = "" + tienda.getProductoFromPosition(i).getImagen() + "";//Para identificar con que producto esta asociado
